    ...
    <div class="lg:col-span-1 space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-gray-700">
            ...
            <form id="send-alert-form" action="{{ route('admin.employees.send-alert', $employee->id) }}" method="POST" class="mt-4">
                @csrf
                <button type="submit" id="send-alert-btn" 
                    @if(!isset($attendance)) disabled @endif
                    class="w-full @if(isset($attendance)) bg-red-600 hover:bg-red-700 @else bg-gray-400 cursor-not-allowed @endif text-white font-bold py-2 px-4 rounded-lg flex items-center justify-center gap-2 transition-colors duration-200">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
                    <span id="send-alert-text">@if(isset($attendance)) Send Urgent Alert @else Employee Offline @endif</span>
                </button>
            </form>
        </div>
    </div>
