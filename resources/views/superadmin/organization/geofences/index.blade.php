...
<div class="flex justify-between items-center mb-6">
    ...
    <div class="flex flex-col md:flex-row items-center gap-4">
        <!-- Search Bar -->
        <form action="{{ route('superadmin.organizations.geofences.index', $org->id) }}" method="GET" class="w-full md:w-auto">
            ...
            @if(request('search'))
                <a href="{{ route('superadmin.organizations.geofences.index', $org->id) }}" class="block mt-2 text-xs text-red-500 hover:text-red-700 font-medium">Clear search</a>
            @endif
        </form>

        <a href="{{ route('superadmin.organizations.geofences.create', $org->id) }}" class="w-full md:w-auto flex items-center justify-center px-5 py-2.5 bg-navy text-white font-bold rounded-xl shadow-md hover:bg-[#233554] transition-all transform hover:-translate-y-0.5">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm space-x-3">
                        ...
                        <form action="{{ route('superadmin.organizations.geofences.destroy', [$org->id, $geofence]) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
