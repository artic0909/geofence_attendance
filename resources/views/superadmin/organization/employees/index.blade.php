...
<div class="flex justify-between items-center mb-6">
    ...
    <div class="flex flex-col md:flex-row items-center gap-4">
        <!-- Search Bar -->
        <form action="{{ route('superadmin.organizations.employees.index', $org->id) }}" method="GET" class="w-full md:w-auto">
            ...
            @if(request('search'))
                <a href="{{ route('superadmin.organizations.employees.index', $org->id) }}" class="block mt-2 text-xs text-red-500 hover:text-red-700 font-medium">Clear search</a>
            @endif
        </form>

        <a href="{{ route('superadmin.organizations.employees.create', $org->id) }}" class="w-full md:w-auto flex items-center justify-center px-5 py-2.5 bg-navy text-white font-bold rounded-xl shadow-md hover:bg-[#233554] transition-all transform hover:-translate-y-0.5">
...
<div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
            ...
            <tbody class="divide-y divide-gray-100 italic-last-row">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm space-x-3">
                        <a href="{{ route('superadmin.organizations.employees.track', [$org->id, $employee]) }}" target="_blank" class="text-green-600 hover:text-green-800 font-bold bg-green-50 px-2 py-1 rounded border border-green-100 italic transition-all shadow-sm">Track Live</a>
                        <a href="{{ route('superadmin.organizations.employees.edit', [$org->id, $employee]) }}" class="text-blue-500 hover:text-blue-700 font-medium">Edit</a>
                        <form action="{{ route('superadmin.organizations.employees.destroy', [$org->id, $employee]) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
    ...
    @else
        <div class="p-12 text-center">
            ...
            <div class="mt-6">
                <a href="{{ route('superadmin.organizations.employees.create', $org->id) }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">
                    Add Employee
                </a>
            </div>
        </div>
